"""Build the gemini-bright-vertex demo video end-to-end."""
...
def base(img, d, title=None, eyebrow=None):
    ...
    d.text((48, H - 44), "gemini-bright-vertex", font=font(22), fill=FG)
    d.text((W - 620, H - 44), "github.com/MukundaKatta/gemini-bright-vertex", font=font(22), fill=FG_MUTED)
    if eyebrow:
...
def draw_title(img, d):
    ...
    d.rectangle([(0, H - 56), (W, H)], fill=PANEL)
    d.text((48, H - 44), "github.com/MukundaKatta/gemini-bright-vertex", font=font(22), fill=FG_MUTED)
    d.text((W - 270, H - 44), "Apache 2.0", font=font(22), fill=FG_MUTED)
    d.text((96, 320), "gemini-bright-vertex", font=font(120), fill=FG)
...
def draw_code(img, d):
    ...
    code = (
        ...
        "from gemini_bright_vertex.agent import _mongo_toolset\n"
        "\n"
        "agent = LlmAgent(\n"
...
def draw_close(img, d):
    d.rectangle([(0, 0), (W, H)], fill=BG)
    d.text((96, 200), "gemini-bright-vertex", font=font(96), fill=FG)
    d.rectangle([(96, 320), (340, 330)], fill=ACCENT)
    d.text((96, 370), "github.com/MukundaKatta/gemini-bright-vertex", font=font(38, mono=True), fill=ACCENT)
    d.text((96, 450), "gemini-bright-vertex-1029931682737.us-central1.run.app", font=font(36, mono=True), fill=ACCENT_2)
...
def main():
    outdir = Path(sys.argv[1]) if len(sys.argv) > 1 else Path.home() / "gemini-bright-vertex" / ".video-build"
